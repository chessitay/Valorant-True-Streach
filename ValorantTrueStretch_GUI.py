...
def process_gus(path: Path, target_x, target_y, apply_changes, label, log_func):
    """Process GameUserSettings.ini file - matches original script logic exactly"""
    if not path.is_file():
...
class ProfessionalApp(tk.Tk):
    ...
    def get_targets_and_check(self, nx, ny, force=False):
        """Get target files and perform native check - matches original script logic"""
        ...
        # Native check - exactly as in original script
